...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 01, 2021 at 09:28 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `countAllPartsSummary` ()  NO SQL
BEGIN
DECLARE sum_variable DOUBLE;

START TRANSACTION;

SELECT COUNT(ram_id) INTO @ram_sum FROM ram;
SET sum_variable = @ram_sum;
SELECT COUNT(videocard_id) INTO @videocard_sum FROM videocard;
SET sum_variable = sum_variable + @videocard_sum;
SELECT COUNT(cpu_id) INTO @cpu_sum FROM cpu;
SET sum_variable = sum_variable + @cpu_sum;

UPDATE summary SET current_summary = sum_variable ,time_currently = now() WHERE summary_id = 1;

COMMIT;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllCPU` ()  BEGIN
	SELECT COUNT(cpu_id) AS "CPU-k száma a CPU táblában" FROM cpu;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllGPU` ()  NO SQL
BEGIN

SELECT COUNT(videocard_id) AS "Videókártyák száma a GPU táblázatban" FROM videocard;
...
--
-- Table structure for table `gpumem`
--
...
--
-- Stand-in structure for view `gskill_rams_speed_above_average`
...
--
-- Table structure for table `ramspeed`
--
...
--
-- Dumping data for table `ramspeed`
--
...
--
-- Stand-in structure for view `viewallryzen7cpus`
-- (See below for the actual view)
--
...
--
-- Structure for view `gskill_rams_speed_above_average`
--
...
--
-- Structure for view `viewallryzen7cpus`
--
...
--
-- AUTO_INCREMENT for table `gpumem`
--
ALTER TABLE `gpumem`
...
--
-- AUTO_INCREMENT for table `ramspeed`
--
ALTER TABLE `ramspeed`
  MODIFY `ramspeed_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

--
-- AUTO_INCREMENT for table `sockettype`
--
ALTER TABLE `sockettype`
...
--
-- Constraints for table `videocard`
--
ALTER TABLE `videocard`
